...
class MT5Interface:
    ...
    def send_market_order(self, symbol, order_type, volume, sl, tp, magic, comment):
        ...
        if res and res.retcode == mt5.TRADE_RETCODE_DONE:
            ui_log('success', f"✅ FILLED: {order_type.upper()} {symbol} | Vol: {request['volume']} | SL: {request['sl']}")
            return True
        else:
            ErrorManager.handle_mt5_error(res.retcode if res else 0, symbol, f"MARKET_{order_type.upper()}")
            return False
...
class LiveSignalEngine:
    ...
    def warmup_strategies(self):
        ...
        for s_config in self.config['strategy_instances']:
            ...
            for i in range(5, len(df) + 1):
                ...
                # Position Management Simulation with GAP logic
    ...
    def process_entries(self):
        for s_config in self.config['strategy_instances']:
            try:
                ...
                ErrorManager.catch_strategy_error(s_config.get('strategy_id', 'Unknown'), "process_entries_wrapper", e)
    ...
    def _process_single_strategy_entry(self, s_config):
        ...
        # Edge Detection: Broker closed position externally
        ...
        # ==============================================================================
        # 1. 👻 GHOST RECOVERY LOGIC (Robust execution)
        # ==============================================================================
                ...
                if has_pending:
                    ...
                    if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                        ...
                        if retries < 1: 
                            ...
                            ui_log('error', f"❌ [CRITICAL] 2nd attempt failed. Killing Ghost Memory. CHECK MT5 MANUALLY!")
                            self.active_recovery_trades[strat_id] = None
                            if hasattr(strat, 'on_exit'): ErrorManager.safe_execute(strat_id, 'on_exit', strat.on_exit)
                else:
                    ui_log('warning', f"👻 [RECOVERY CANCELLED] Market hit TP. Rhythm Unlocked!")
                    self.active_recovery_trades[strat_id] = None
                    if hasattr(strat, 'on_exit'): ErrorManager.safe_execute(strat_id, 'on_exit', strat.on_exit)
            else:
                if not has_pending:
                    ...
                    if df_recovery is not None:
                        ...
                        if execute_market:
                            ...
                            else: self.active_recovery_trades[strat_id] = None
                        else:
    ...
    def _execute_limit_order(self, trade_type, entry_anchor, sl_anchor, tp_anchor, s_config, df):
        ...
        if desired_lot > 0:
            ...
            if final_lot > 0:
                ui_log('warning', f"🎣 [MT5 LIMIT PLACED] Waiting for Pullback: {calc_entry} | Vol: {final_lot}")
                ...
                request = {
                    ...
                    "comment": limit_comment, "type_time": mt5.ORDER_TIME_GTC
                }
                res = mt5.order_send(request)
        ...
        while True:
            try:
                ...
                if self.target_lock.check_and_lock(self.mt5_interface):
                    ui_log('warning', "🔒 Target Lock Triggered! Engine shutting down.")
                    mt5.shutdown()
                    break
